...
```
2차원 평면 상에서 두 점 사이의 거리를 구하는 공식인 (x2-x1)^2+(y2-y1)^2을 이용한다. double형 변수 distance를 선언해 구한 거리의 제곱근을 저장하도록 한다. 각각 점들의 거리 중 최솟값을 구하기 위해 for반복문과 if조건문을 이용해 distance들을 비교한다. distance의 값이 min2 변수의 값 보다 작을 경우 min2에 distance를 저장하도록 하며 최솟값을 구한다.
